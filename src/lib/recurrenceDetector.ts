...
export function detectRecurring(transactions: Transaction[]): RecurrenceSuggestion[] {
  ...
  for (const [, txs] of groups) {
    ...
    // Check if they appear in consecutive or near-consecutive months
    const months = txs.map((t) => parseISO(t.date).getMonth());
    const uniqueMonths = [...new Set(months)].sort((a, b) => a - b);
    const consecutiveCount = countConsecutive(uniqueMonths);
    const confidence = Math.min(consecutiveCount / Math.max(uniqueMonths.length, 3), 1);
