...
export function PixQRCode({ brCode, brCodeBase64, expiresAt, amount }: Props) {
  ...
  if (isExpired) {
    return (
      <div className="flex flex-col items-center gap-3 p-6 bg-white rounded-xl border border-[#e2e8f0]">
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-6 bg-white rounded-xl border border-[#e2e8f0]">
      ...
      <div className="flex items-center gap-2 w-full max-w-sm">
        ...
        <button
          onClick={handleCopy}
          className="px-3 py-2 bg-[#3b82f6] text-white rounded-lg text-xs font-medium hover:bg-[#2563eb] cursor-pointer flex items-center gap-1.5 shrink-0 transition-colors"
        >
