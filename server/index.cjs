...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url === "/health") { res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ status: "ok" })); return; }
  ...
  if (req.method === "POST" && req.url === "/api/auth/welcome") {
    try {
      ...
      if (!email) { res.writeHead(400, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "email obrigatorio" })); return; }
    ...
    } catch { res.writeHead(400, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "JSON invalido" })); }
    return;
  }
  ...
  if (req.method === "POST" && req.url === "/api/sub/create") {
    try {
      ...
      if (!userEmail || !items.length) { res.writeHead(400, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "userEmail e items obrigatorios" })); return; }
    ...
    } catch (err) { res.writeHead(500, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: err.message || "erro" })); }
    return;
  }
  ...
  if (req.method === "POST" && req.url === "/api/sub/cancel") {
    ...
    const userEmail = String(authUser.email || "").trim().toLowerCase();
    const sub = await getSubscriptionByEmail(userEmail);
    if (!sub) { res.writeHead(404, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "assinatura nao encontrada" })); return; }
    ...
    res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true }));
    return;
  }
  ...
  if (req.method === "POST" && req.url.startsWith("/api/webhooks/abacate")) {
    try {
      ...
      if (!event?.id || !event?.event) { res.writeHead(400, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "payload invalido" })); return; }

      const requestUrl = new URL(req.url, "http://localhost");
      const webhookSecret = String(requestUrl.searchParams.get("webhookSecret") || "");
      if (ABACATE_WEBHOOK_SECRET && webhookSecret !== ABACATE_WEBHOOK_SECRET) { res.writeHead(401, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "webhookSecret invalido" })); return; }
      const sigHeader = req.headers["x-webhook-signature"] || req.headers["x-abacate-signature"];
      if (!verifyAbacateSignature(rawBody, String(sigHeader || ""))) { res.writeHead(401, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "assinatura invalida" })); return; }

      const isNew = await markWebhookEventProcessed(event.id);
      if (!isNew) { res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true, deduplicated: true })); return; }
      ...
      if (eventEmail) {
        ...
        if (!previous) { res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true, ignored: true })); return; }
  ...
  if (req.method === "GET" && req.url === "/api/admin/plans") {
    ...
    if (!isSuperadmin(u)) { res.writeHead(403, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "restrito" })); return; }
    try {
      const db = getFirestore(); const snap = await db.collection("plans").orderBy("createdAt", "desc").get();
      res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true, data: snap.docs.map(d => ({ id: d.id, ...d.data() })) }));
  ...
  if (req.method === "POST" && req.url === "/api/admin/plans") {
    ...
    try {
      ...
      res.writeHead(201, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true, data: { id: ref.id } }));
    } catch (e) { res.writeHead(500, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: e.message })); }
    return;
  }
  ...
  if (req.method === "DELETE" && req.url.startsWith("/api/admin/plans/")) {
    const u = await authRequired(req, res); if (!u) return;
    if (!isSuperadmin(u)) { res.writeHead(403, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "restrito" })); return; }
    try { const db = getFirestore(); await db.collection("plans").doc(req.url.split("/api/admin/plans/")[1]).delete(); res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true })); } catch (e) { res.writeHead(500, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: e.message })); }
    return;
  }
  ...
  if (req.method === "GET" && req.url === "/api/admin/subscriptions") {
    const u = await authRequired(req, res); if (!u) return;
    if (!isSuperadmin(u)) { res.writeHead(403, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "restrito" })); return; }
    try { const subs = await getAllSubscriptions(); res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true, data: subs })); } catch (e) { res.writeHead(500, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: e.message })); }
    return;
  }
  ...
  if (req.method === "POST" && req.url === "/api/admin/subscriptions/assign") {
    ...
    try {
      ...
      if (!targetEmail || !planId) { res.writeHead(400, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "targetEmail e planId obrigatorios" })); return; }
      const db = getFirestore(); const planDoc = await db.collection("plans").doc(planId).get();
      if (!planDoc.exists) { res.writeHead(404, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "plano nao encontrado" })); return; }
  ...
  if (req.method === "PUT" && req.url.startsWith("/api/admin/members/") && req.url.endsWith("/permissions")) {
    ...
    try {
      ...
      if (!accountId || !permissions) { res.writeHead(400, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "accountId e permissions obrigatorios" })); return; }
      const db = getFirestore();
      const memberRef = db.collection("accounts").doc(accountId).collection("members").doc(memberUid);
      const memberDoc = await memberRef.get();
      if (!memberDoc.exists) { res.writeHead(404, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "membro nao encontrado" })); return; }
      const callerDoc = await db.collection("accounts").doc(accountId).collection("members").doc(u.uid).get();
      if (!callerDoc.exists || !['owner', 'admin'].includes(callerDoc.data().role)) { res.writeHead(403, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "sem permissao" })); return; }
  ...
  if (req.method === "GET" && req.url === "/api/admin/reports") {
    const u = await authRequired(req, res); if (!u) return;
    if (!isSuperadmin(u)) { res.writeHead(403, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "restrito" })); return; }
    try { const db = getFirestore(); const snap = await db.collection("reports").orderBy("createdAt", "desc").get(); res.writeHead(200, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ success: true, data: snap.docs.map(d => ({ id: d.id, ...d.data() })) })); } catch (e) { res.writeHead(500, { ...corsHeaders(), "Content-Type": "application/json" }); res.end(JSON.stringify({ error: e.message })); }
    return;
  }
  ...
  if (req.method === "POST" && req.url === "/api/ai/chat") {
    try { const parsed = await readJsonBody(req); parsed.stream ? await proxyStream(req, res, parsed) : await proxyNonStream(req, res, parsed); } catch { res.writeHead(400, corsHeaders()); res.end(JSON.stringify({ error: "JSON invalido" })); }
    return;
  }
