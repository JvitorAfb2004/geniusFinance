    ...
    <div>
      ...
      {(() => {
        ...
        const currentClosing = monthlyClosings.find(
          (c) => c.context === activeContext && c.year === year && c.month === month
        );
        if (!currentClosing) {
          return (
        ...
        if (currentClosing.status === 'CLOSED') {
          return (
            <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-xs text-emerald-800 font-medium flex items-center gap-2">
              ...
              Mês de {format(selectedMonth, 'MMMM/yyyy', { locale: ptBR })} fechado em {format(new Date(currentClosing.closedAt), "dd/MM/yyyy", { locale: ptBR })}.
            </div>
          );
        }
        return null;
