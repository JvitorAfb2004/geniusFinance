...
const CAT_COLORS = ['#4fb8b2', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899'];

export default function GoalsView() {
  const { goals, addGoal, updateGoal, deleteGoal } = useFinance();
  const [showForm, setShowForm] = useState(false);
  const [editingGoalId, setEditingGoalId] = useState<string | null>(null);
  const [form, setForm] = useState({ name: '', targetAmount: '', currentAmount: '', deadline: '', category: 'SAVINGS' as FinancialGoal['category'], color: '#4fb8b2' });

  function resetForm() {
    setForm({ name: '', targetAmount: '', currentAmount: '', deadline: '', category: 'SAVINGS', color: '#4fb8b2' });
