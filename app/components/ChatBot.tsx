...
${sortedMonthlyExpenses.map((e) => `  ${e.name}: ${formatCurrency(e.monthly)}/mes (${e.months}x no ano)`).join('\n') || '  nenhum'}`;
  ...
  async function handleSend(text?: string) {
    ...
    try {
      const response = await chatStream(apiMessages,
        (token) => {
          setMessages((prev) => {
            const updated = [...prev];
      ...
      setMessages((prev) => {
        const updated = [...prev];
        const last = updated[updated.length - 1];
        updated[updated.length - 1] = { ...last, content: response.content || last.content };
        return updated;
      });
    } catch (e: unknown) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-[420px] max-w-[calc(100vw-2rem)] h-[600px] max-h-[calc(100vh-2rem)] clay shadow-2xl flex flex-col overflow-hidden">
          {/* Header */}
