...
export function Header({
  ...
  return (
    <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center px-4 sm:px-6 py-4 sm:py-4.5 shrink-0 w-full gap-4 bg-surface border-b border-border transition-colors">
      <div className="flex items-center gap-3 sm:gap-6 w-full sm:w-auto">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {activeScope.type === 'ACCOUNT' && (
            <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-[0.62rem] font-bold bg-primary-light text-primary-dark border border-primary/20 uppercase tracking-wider">
              Corporativo
            </span>
          )}
        </div>
        ...
        {/* Month Selector */}
        <div className="flex items-center gap-1.5 text-sm font-medium clay-btn px-1.5 py-1 ml-auto sm:ml-0 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
          <button
            onClick={() => setSelectedMonth(subMonths(selectedMonth, 1))}
            className="p-1 text-slate-400 hover:text-slate-700 hover:bg-black/5 rounded-lg transition-all border-none bg-transparent cursor-pointer"
          >
      ...
      <div className="flex gap-2 w-full sm:w-auto items-center">
            ...
            return (
              <button
                ...
                className={`whitespace-nowrap px-3.5 py-1.5 rounded-lg text-[0.8rem] font-semibold transition-all border-none cursor-pointer inline-flex items-center justify-center gap-1.5 shrink-0 ${
                  isActive
                    ? 'bg-white/80 text-text-primary shadow-[inset_1px_-1px_3px_rgba(0,0,0,0.06)] font-semibold'
        ...
        <button
          ...
          className="h-[36px] shrink-0 px-2.5 sm:px-3 clay-btn text-[0.78rem] font-bold text-slate-600 hover:text-slate-800 hover:brightness-95 transition-all cursor-pointer inline-flex items-center gap-1.5"
