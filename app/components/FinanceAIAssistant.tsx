...
export function FinanceAIAssistant() {
  ...
  async function confirm(proposal: AgentProposal) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: "assistant", content: `Concluído. ${JSON.stringify(result.result)}` }]);
    } catch (error) {
  ...
  return (
    <>
      {!open && (
        <button onClick={() => setOpen(true)} aria-label="Abrir assistente financeiro" title="Assistente financeiro" className="fixed bottom-20 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-white shadow-lg transition hover:bg-primary-hover active:scale-95 lg:bottom-6 lg:right-6">
      ...
      {open && (
        <section aria-label="Assistente financeiro" className="fixed inset-0 z-50 flex flex-col bg-surface lg:inset-auto lg:bottom-6 lg:right-6 lg:h-[min(600px,calc(100vh-3rem))] lg:w-[min(440px,calc(100vw-2rem))] lg:rounded-lg lg:border lg:border-border lg:shadow-2xl">
          <header className="flex items-center justify-between border-b border-border bg-slate-900 px-4 py-3 text-white lg:rounded-t-lg">
            ...
            <button onClick={() => setOpen(false)} aria-label="Fechar assistente" className="rounded p-1 text-slate-300 hover:text-white"><X className="h-5 w-5" /></button>
          </header>
          <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4" aria-live="polite">
            ...
            {messages.map((message, index) => <div key={`${index}-${message.content}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[88%] whitespace-pre-wrap rounded-lg px-3 py-2 text-sm leading-relaxed ${message.role === "user" ? "bg-primary text-white" : "border border-border bg-white text-text-primary"}`}>{message.content}
              {message.proposal && <div className="mt-3 space-y-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-xs text-amber-950"><p className="font-semibold">Confirmação necessária</p><p>Operação: {message.proposal.action}</p>{Object.entries(message.proposal.arguments).map(([key, value]) => <p key={key}><strong>{key}:</strong> {formatPreview(value)}</p>)}<div className="flex gap-2 pt-1"><button onClick={() => confirm(message.proposal!)} disabled={confirming !== null} className="inline-flex items-center gap-1 rounded bg-primary px-2.5 py-1.5 font-semibold text-white disabled:opacity-50"><Check className="h-3.5 w-3.5" />{confirming === message.proposal.id ? "Executando" : "Confirmar"}</button><button onClick={() => setMessages((current) => current.map((item) => item.proposal?.id === message.proposal?.id ? { ...item, proposal: undefined, content: "Alteração cancelada." } : item))} disabled={confirming !== null} className="rounded border border-amber-300 bg-white px-2.5 py-1.5 font-semibold disabled:opacity-50">Cancelar</button></div></div>}
            </div></div>)}
          ...
          <form onSubmit={(event) => { event.preventDefault(); void send(); }} className="flex gap-2 border-t border-border bg-white p-3 lg:rounded-b-lg"><input ref={inputRef} value={input} onChange={(event) => setInput(event.target.value)} disabled={sending} maxLength={4000} placeholder="Pergunte sobre suas finanças..." className="clay-input min-w-0 flex-1 px-3 py-2 text-sm" /><button type="submit" disabled={sending || !input.trim()} aria-label="Enviar mensagem" className="rounded-md bg-primary px-3 text-white disabled:opacity-50"><Send className="h-4 w-4" /></button></form>
        </section>
      )}
    </>
  );
